...
def test_stock_mass_update_ok(db_session):
    item_model1 = __create_dummy_component_item(db_session, mpn="test1", manufacturer="test1")
    item_model2 = __create_dummy_component_item(db_session, mpn="test2", manufacturer="test2")

    location_model_1 = InventoryLocationModel(name='location1', description='description1', dici='dummydici1')
    location_model_2 = InventoryLocationModel(name='location2', description='description2', dici='dummydici2')
    db_session.add(location_model_1)
    db_session.add(location_model_2)
    db_session.commit()

    item_stock_loc_1 = InventoryItemLocationStockModel(location_id=location_model_1.id, item_id=item_model1.id,
                                                       actual_stock=0.0, stock_min_level=0.0,
                                                       stock_notify_min_level=0.0)
    item_stock_loc_2 = InventoryItemLocationStockModel(location_id=location_model_2.id, item_id=item_model2.id,
                                                       actual_stock=0.0, stock_min_level=0.0,
                                                       stock_notify_min_level=0.0)
    db_session.add(item_stock_loc_1)
    db_session.add(item_stock_loc_2)
    db_session.commit()

    movement = MassStockMovement(reason='test move', comment='test comment', movements=[
        SingleStockMovement(item_id=item_model1.id, location_id=location_model_1.id, quantity=3),
        SingleStockMovement(item_id=item_model2.id, location_id=location_model_2.id, quantity=1),
    ])
